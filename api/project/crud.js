...
module.exports = function(model, images, oauth2) {
   ...
   * GET /project/add
   *
   * Display a page of project (up to ten at a time).
   */
  ...
  router.get('/mine', oauth2.required, function list(req, res, next) {
    model.listBy(req.session.profile.id, 10, req.query.pageToken, function(err, entities, cursor, apiResponse) {
   ...
   */
    // [START add]
  router.post('/add', images.multer.single('image'), images.sendUploadToGCS, function insert(req, res, next) {
      var data = req.body;
   ...
   * Update a project.
   */
  router.post('/:project/edit', images.multer.single('image'), images.sendUploadToGCS, function update(req, res, next) {
      var data = req.body;
