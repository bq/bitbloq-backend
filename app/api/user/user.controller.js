...
exports.create = function(req, res) {
    if (
        ...
        (req.body.password && req.body.recoveryMail && req.body.tutor.email)
    ) {
...
function sendEmailTutorAuthorization(user, next) {
    ...
    var authorizationUrl =
        config.client_domain + '/#/under14authorization/' + token;
                    ...
                    if (req.user) {
                        ...
                        searchSocialByEmail(user, function(err, localUser) {
                            if (err) {
                                ...
                                if (!localUser) {
                                    var userData = generateSocialUser(
                                        provider,
                                        user
                                    );
                                    if (register) {
                                        ...
                                        async.waterfall(
                                            [
                                                ...
                                                function(
                                                    ...
                                                    ImageFunctions.downloadAndUploadImage(
                                                        avatarUrl,
                                                        'images/avatar/' +
                                                            user._id.toString(),
                                                        function(err) {
                                                ...
                                                function(user, saveCallback) {
                                                    if (
                                                        newUser.needValidation
                                                    ) {
                                                        sendEmailTutorAuthorization(
                                                            user,
                                                            function(err) {
                                                                if (err) {
                                                                    console.log(
                                                                    ...
                                                                    err.code = utils.getValidHttpErrorCode(
                                                                        err
                                                                    );
                                                                    res
                                                                        .status(
                ...
                if (user) {
                    if (_hardwareIsEmpty(user.hardware)) {
                        if (user.seeBoardsUnderDevelopment) {
                            HardwareFunctions.getAllHardware(function(
                                err,
                                hardware
                            ) {
                            ...
                            HardwareFunctions.getDefault(function(
                                err,
                                hardware
                            ) {
                                user.hardware = hardware;
            ...
            if (user) {
                ...
                user.update(userToUpdate, function(err) {
                    if (err) {
                        ...
                        HardwareFunctions.getHardware(
                            userToUpdate.hardware,
                            function(err, userHardware) {
                                if (err) {
                                    console.log(err);
                                    err.code = utils.getValidHttpErrorCode(err);
                                    res.status(err.code).send(err);
...
function _hardwareIsEmpty(hardware) {
    return (
        !hardware ||
        (hardware.robots.length === 0 &&
            hardware.boards.length === 0 &&
